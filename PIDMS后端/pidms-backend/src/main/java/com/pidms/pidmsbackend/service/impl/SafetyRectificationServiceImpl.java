...
import com.pidms.pidmsbackend.convert.SafetyRectificationConvert;
...
@Slf4j
@Service
public class SafetyRectificationServiceImpl implements SafetyRectificationService {
    ...
    @Resource
    private SafetyRectificationConvert safetyRectificationConvert;
    ...
    public Result<PageInfo<SafetyRectificationVO>> pageQuery(SafetyRectificationQueryParam queryParam) {
        ...
        log.info("分页查询安全整改单：pageNum = {}, pageSize = {}, 参数 = {}", pageNum, pageSize, queryParam);

        QueryWrapper<SafetyRectification> queryWrapper = new QueryWrapper<>();
        // 高级搜索：整改状态精确匹配
        ...
        Page<SafetyRectification> page = new Page<>(pageNum, pageSize);
        Page<SafetyRectification> resultPage = safetyRectificationMapper.selectPage(page, queryWrapper);
        List<SafetyRectification> records = resultPage.getRecords();
        log.info("分页查询安全整改单结果条数：{}", records.size());

        PageInfo<SafetyRectificationVO> pageInfo = new PageInfo<>();
        ...
        pageInfo.setList(fillProjectName(safetyRectificationConvert.entityListToVoList(records)));
        return Result.success(pageInfo);
    }

    @Override
    public Result<SafetyRectificationVO> getById(Long id) {
        SafetyRectification entity = safetyRectificationMapper.selectById(id);
        if (entity == null) {
            return Result.error("安全整改单不存在，id = " + id);
        }
        return Result.success(fillProjectName(Collections.singletonList(safetyRectificationConvert.entityToVo(entity))).get(0));
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public Result<Long> add(SafetyRectificationDTO dto) {
        ...
        SafetyRectification entity = safetyRectificationConvert.dtoToEntity(dto);
        ...
        safetyRectificationMapper.insert(entity);
        log.info("新增安全整改单成功，id = {}", entity.getId());
        return Result.success(entity.getId());
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public Result<Long> update(Long id, SafetyRectificationDTO dto) {
        SafetyRectification exist = safetyRectificationMapper.selectById(id);
        if (exist == null) {
            return Result.error("安全整改单不存在，id = " + id);
        }
        ...
        safetyRectificationMapper.updateById(entity);
        log.info("编辑安全整改单成功，id = {}", id);
    ...
    public Result<Boolean> delete(Long id) {
        if (safetyRectificationMapper.selectById(id) == null) {
            return Result.error("安全整改单不存在，id = " + id);
        }
        int rows = safetyRectificationMapper.deleteById(id);
        log.info("删除安全整改单成功，id = {}", id);
    ...
    private boolean isRectificationNoUnique(String rectificationNo, Long excludeId) {
        Long count = safetyRectificationMapper.selectCount(Wrappers.<SafetyRectification>lambdaQuery()
                .eq(SafetyRectification::getRectificationNo, rectificationNo)
                .ne(excludeId != null, SafetyRectification::getId, excludeId));
     ...
     */
    private List<SafetyRectificationVO> fillProjectName(List<SafetyRectificationVO> voList) {
        if (CollectionUtils.isEmpty(voList)) {
            return voList;
        }
        List<Long> projectIds = voList.stream()
                .map(SafetyRectificationVO::getProjectId)
    ...
    private PageInfo<SafetyRectificationVO> emptyPage(long pageNum, long pageSize) {
        PageInfo<SafetyRectificationVO> pageInfo = new PageInfo<>();
