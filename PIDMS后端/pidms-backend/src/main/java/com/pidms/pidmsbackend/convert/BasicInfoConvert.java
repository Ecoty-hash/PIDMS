package com.pidms.pidmsbackend.convert;

import com.pidms.pidmsbackend.dto.BasicInfoDTO;
import com.pidms.pidmsbackend.entity.BasicInfo;
import com.pidms.pidmsbackend.vo.BasicInfoVO;
import org.mapstruct.Mapper;
import org.mapstruct.Mapping;

import java.util.List;

/**
 * 基础资料 转换器
 * <p>DTO→Entity 忽略审计字段（由 Service 补全）。</p>
 */
@Mapper(componentModel = "spring")
public interface BasicInfoConvert {

    @Mapping(target = "id", ignore = true)
    @Mapping(target = "createBy", ignore = true)
    @Mapping(target = "createTime", ignore = true)
    @Mapping(target = "updateBy", ignore = true)
    @Mapping(target = "updateTime", ignore = true)
    BasicInfo dtoToEntity(BasicInfoDTO dto);

    BasicInfoVO entityToVo(BasicInfo entity);

    List<BasicInfoVO> entityListToVoList(List<BasicInfo> list);
}
