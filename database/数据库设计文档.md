...
| 配套脚本 | `database/init/01_create_database.sql`、`02_schema.sql`、`03_seed.sql`<br>增量：`database/upgrade/11_basic_management.sql`、`12_seed_basic_management.sql`、`13_add_progress_responsible_person.sql`、`14_add_progress_parent_and_weight.sql` |
...
| --- | --- | --- | --- | --- |
| `project_id` | BIGINT UNSIGNED | 否 | — | 项目ID → `pm_project.id` |
| `parent_id` | BIGINT UNSIGNED | 是 | NULL | 父节点ID → `cm_progress.id`，NULL 为顶层节点；层级不限（多层子节点），不加外键、由应用层整棵子树级联删除 |
...
| `completion_percent` | DECIMAL(5,2) | 否 | 0.00 | 完成百分比(0-100)；有子节点的父节点该值不参与计算，由子节点加权汇总 |
| `weight` | DECIMAL(5,2) | 是 | NULL | 占总进度百分比(0-100)，仅叶子节点填写；父节点的权重由子节点汇总（读取时计算，不落库） |
| `progress_status` | VARCHAR(20) | 否 | in-progress | in-progress/completed/delayed |
| `responsible_person` | VARCHAR(50) | 是 | NULL | 节点负责人（真实姓名），候选来源为在职内部人员全量名单，本项目成员置顶 |
| `remark` | VARCHAR(500) | 是 | NULL | 备注 |

索引：`idx_progress_project`、`idx_progress_parent`、`idx_progress_status`。

节点层级与进度口径（唯一实现见后端 `ProjectProgressCalculator`）：叶子节点完成度取手填的 `completion_percent`；父节点完成度 = Σ(子节点权重 × 子节点完成度) ÷ Σ子节点权重；项目实际进度 = Σ(顶层节点权重 × 顶层节点完成度) ÷ Σ顶层节点权重。未填 `weight` 的节点按「同组未填节点平分剩余份额」处理，整组都未填即为均分，因此存量数据不受影响。期望值（理论进度）按计划工期线性推进：(今天 − 计划开始) ÷ (计划结束 − 计划开始)，封顶 100%，父节点与项目级按同一权重汇总。
